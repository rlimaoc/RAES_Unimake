...
namespace Uni.Business.DFe.Servicos
{
    ...
    [ProgId("Uni.Business.DFe.Servicos.Configuracao")]
    [ComVisible(true)]
#endif
    public class Configuracao
    {
        ...
        public void Load(string nomeTagServico)
        {
            ...
            {
                var doc = new XmlDocument();
                var arq = Configuration.ArquivoConfigGeral;
                var xml = LoadXmlConfig(arq);
                doc.Load(xml);
                ...
                {
                    throw new Exception((CodigoConfig.ToString().Length <= 2 ? "Unidade Federativa" : "Município") + " (" + CodigoConfig + ") não está implementado na DLL Uni.DFe. Entre em contato com o suporte para solicitar a implementação.");
                }
            }
        }
