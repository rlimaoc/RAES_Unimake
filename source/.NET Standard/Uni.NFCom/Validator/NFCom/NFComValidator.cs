...
using Uni.NFCom.Validator.Abstractions;
using Uni.NFCom.Xml.NFCom;
/*
using Unimake;
namespace Uni.NFCom.Validator.NFCom
{
    /// <summary>
    ...
    public class NFComValidator : XmlValidatorBase
    {
            ...
            }).ValidateTag(element => element.NameEquals(nameof(Rastro.DFab)) && element.Parent.NameEquals(nameof(Rastro)) && element.Parent.Parent.NameEquals(nameof(Prod)), Tag =>
            {
                ...
                if (!ValidDate(Tag.Value))
                {
                    ThrowHelper.Instance.Throw(new ValidatorDFeException("O conteúdo da data de fabricação/produção do detalhamento de produto sujeito a rastreabilidade está incorreta. Conteúdo informado: " + Tag.Value + "." +
                        " [Item: " + nItem + "] [cProd: " + cProd + "] [xProd: " + xProd + "] [TAG: <dFab> do grupo de tag <infNFCom><det><prod><rastro>]"));
                }
            }).ValidateTag(element => element.NameEquals(nameof(Rastro.DVal)) && element.Parent.NameEquals(nameof(Rastro)) && element.Parent.Parent.NameEquals(nameof(Prod)), Tag =>
            {
                ...
                if (!ValidDate(Tag.Value))
                {
                    ThrowHelper.Instance.Throw(new ValidatorDFeException("O conteúdo da data de validade do detalhamento de produto sujeito a rastreabilidade está incorreta. Conteúdo informado: " + Tag.Value + "." +
                        " [Item: " + nItem + "] [cProd: " + cProd + "] [xProd: " + xProd + "] [TAG: <dVal> do grupo de tag <infNFCom><det><prod><rastro>]"));
                }
            }).ValidateTag(element => element.NameEquals(nameof(NFCom)), Tag =>
            {
                //Só vou validar namespace, por hora, se não for arquivo de distribuição, pois tem estados que estão aceitando a tag NFCom sem o namespace e naturalmente o arquivo está sendo distribuindo sem e teremos que aceitar.
                if (Tag.Parent == null || Tag.Parent.Name.LocalName != "nfeProc")
                {
                    if (Tag.GetAttributeValue("xmlns") == null)
                    {
                        ThrowHelper.Instance.Throw(new ValidatorDFeException("TAG <NFCom> deve possuir o atributo de namespace, conforme a seguir: <NFCom xmlns=\"http://www.portalfiscal.inf.br/nfe\">"));
                    }
                }
            }).ValidateTag(element => element.NameEquals(nameof(InfNFCom)) && element.Parent.NameEquals(nameof(NFCom)), Tag =>
            {
                if (Tag.GetAttributeValue("xmlns") != null)
                {
                    ThrowHelper.Instance.Throw(new ValidatorDFeException("Não pode existir o atributo de namespace na tag <infNFCom>. Remova o conteúdo xmlns=\"http://www.portalfiscal.inf.br/nfe\" da tag <infNFCom>. [TAG: <infNFCom> do grupo de tag <NFCom>]"));
                }
            }).ValidateTag(element => element.NameEquals(nameof(AutXML.CPF)) && element.Parent.NameEquals(nameof(AutXML)) && element.Parent.Parent.NameEquals(nameof(InfNFCom)), Tag =>
            {
                if (Tag.Value.Trim() == "00000000000" || string.IsNullOrWhiteSpace(Tag.Value.Trim()))
                {
                    ThrowHelper.Instance.Throw(new ValidatorDFeException("O conteúdo da tag <CPF> das pessoas autorizadas a acessar o XML não é válido. Conteúdo informado: " + Tag.Value + "." +
                        " [TAG: <CPF> do grupo de tag <NFCom><infNFCom><autXML>]"));
                }
            }).ValidateTag(element => element.NameEquals(nameof(AutXML.CNPJ)) && element.Parent.NameEquals(nameof(AutXML)) && element.Parent.Parent.NameEquals(nameof(InfNFCom)), Tag =>
            {
                if (Tag.Value.Trim() == "00000000000000" || string.IsNullOrWhiteSpace(Tag.Value.Trim()))
                {
                    ThrowHelper.Instance.Throw(new ValidatorDFeException("O conteúdo da tag <CNPJ> das pessoas autorizadas a acessar o XML não é válido. Conteúdo informado: " + Tag.Value + "." +
                        " [TAG: <CNPJ> do grupo de tag <NFCom><infNFCom><autXML>]"));
                }
            });
        ...
        /// <returns></returns>
        public override bool CanValidate(XElement element) => XMLUtility.DetectDFeType(element.ToString()) == TipoDFe.NFCom || XMLUtility.DetectDFeType(element.ToString()) == TipoDFe.NFCe;
