 ...
 * Criar objeto para pegar excecao do lado do CSHARP
   ...
   Try
	 ...
	 * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  	   MESSAGEBOX("A1 e A3 - Abrir tela para selecionar o certificado digital que eu desejo trabalhar, certificado que est� instalado no repositorio do windows")
	   
	   oCertSel1 = oCertificado.AbrirTelaSelecao()
	   
     * Voce pode salvar o Thumbprint ou SerialNumber do certificado para salvar em sua base de dados para resgatar ele no futuro no reposit�rio do windows sem precisar abrir tela para selecionar novamente.
	 ...
	 * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	 * Tentar carregar o certificado A1 do Path e pegar a exce��o
	 
	   TRY
          oCertificado.CarregarCertificadoDigitalA1("c:\projetos\certificados\UnimakePV.pfx", "12345678")
	   CATCH TO oErro2
   	    * Deu algum erro na hora de carregar o certificado A1, dentre eles, se for a senha incorreta, a mensagem ser� bem clara para o usu�rio.
	 ...
	 * Agora voc� pode gravar o conte�do da "certBase64" no banco de dados
	 ...
	 * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
     * Certificado A3 - Setar o PIN do A3 para n�o precisar informar manualmente
	 * -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 * Ver se � um certificado A3
	   If oCertA3.IsA3(oCertSel1)
	      MESSAGEBOX("� certificado A3")
		  
		  oCertA3.SetPinPrivateKey(oCertSel1, "123456")
	   else
	      MESSAGEBOX("� certificado A1")
	   endif
   CATCH TO oErro
    * Excecao do FOXPRO
...
Function MostrarDados(oCert, oCertSel)
   ...
   MESSAGEBOX("ID do Certificado"     + chr(13)+chr(10)+ oCert.GetThumbPrint(oCertSel) + chr(13)+chr(10)+chr(13)+chr(10) +;
              ...
              "Certificado vencido?"  + chr(13)+chr(10)+ IIF(oCert.Vencido(oCertSel), "SIM", "N�O")) 
Return