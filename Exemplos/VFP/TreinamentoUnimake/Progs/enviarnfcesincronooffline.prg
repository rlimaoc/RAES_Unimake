* ---------------------------------------------------------------------------------
* Enviar NFCe no modo s�ncro em conting�ncia off-line
* situa��o (consulta via chave da nfe)
* ---------------------------------------------------------------------------------

* - Status de envio do proprio software
* -- 0 - Processo de envio iniciado
* -- 1 - XML gerado, assinado e arquivado
* -- 2 - XML gerado e emitido danfe em conting�ncia offline
* -- 3 - XML enviado
* -- 4 - Retorno recebido e dados importantes arquivados (Protocolo, etc.)
* -- 5 - XML de distribui��o arquivado
* -- 6 - Processo de envio conclu�do com sucesso
* - E se a nota foi duplicada? Com isso n�o vai acontecer, mas se for fa�a o seguinte
...
FUNCTION EnviarNfceSincronoOffLine()
   ...
   lEnvioOffline = .F.
   lConexaoNet = .T. && Temos conex�o com a internet (Executar alguma rotina que teste a internet)
   ...
   DO WHILE .T.
	 * Criar configuracao basica para consumir o servico
	   ...
	   oConfig.TipoEmissao = IIF(lEnvioOffLine,9,1) && 1=Normal 9=Conting�ncia offline
	 ...
	 * Criar a tag <NFe>  
	    ...
	    * Vamos desserializar XML assinado que j� temos no HD ou no banco de dados
...
*                THROW CREATEOBJECT("myException")
             ...
             * Codigo de Status e Motivo
               ...
               IF oAutorizacao.Result.CStat == 104 && 104 = Lote Processado
                   ...
                   * Gravar XML de distribuicao em uma pasta (NFe com o protocolo de autorizacao anexado)
      		       ...
      		       * Pegar a string do XML de distribui��o
                     ...
                     nStatusEnvio = 5 && XML de distribui��o arquivado
                  ...
                  ENDIF
               ELSE
                  IF oAutorizacao.Result.CStat == 108 && Servidor de Processamento est� Paralisado temorariamente ###
                     lEnvioOffline = .T.
                     xJust = "Servidor de processamento da SEFAZ est� paralisado temporariamente - cStat = 108"
                     lSair = .F.
                  ELSE
                     IF oAutorizacao.Result.CStat == 109 && Servidor de Processamento est� Paralisado sem Previs�o de retorno ###
                        lEnvioOffline = .T.
                        xJust = "Servidor de Processamento est� Paralisado sem Previs�o de retorno - cStat = 109"
                        lSair = .F.
                     ENDIF   
                  ENDIF
               ...
               ENDIF
            ELSE
             * Criar configura��o m�nima para consumir o servi�o de consulta protocolo
             ...
             * Consultar a nota pela chave e se autorizada finalizar gerando o XML de distribui��o   
          ...
          *
          * Criar as configura��es para impressao do DANFe.
          *
          ...
          * Disparar a impressao do DANFe
            ...
            lSair = .T.
            lStatusEnvio = 2 && XML gerado e emitido danfe em conting�ncia offline
         ENDIF   
