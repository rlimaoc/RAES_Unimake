* ---------------------------------------------------------------------------------
* Gerar XML da NFe e enviar no modo Ass�ncrono
* ---------------------------------------------------------------------------------
...
*     * crirar tag PISNT
*       oPISNT = CREATEOBJECT("Uni.Business.DFe.Xml.NFe.PISNT")  
 ...
 * Criar objeto para pegar excecao do lado do CSHARP
    ...
    * Codigo de Status e Motivo
	   ...
	   * Criar o objeto de configura��o m�nimo para consumir o servi�o de consulta recibo
       ...
       * Fazer a consulta do recibo do lote enviado	para ver se a(s) nota(s) foi(ram) autorizada(s)
         oConsReciNFe   = CreateObject("Uni.Business.DFe.Xml.NFe.ConsReciNFe")
       ...
       * Vamos verificar se o status do lote enviado est� como processado com sucesso
	     IF oRetAutorizacao.Result.CStat = 104 && Lote processado com sucesso
          * Informar para o objeto de autoriza��o qual � o resultado das notas. � necess�rio esta informa��o para que 
          * a classe tenha informa��es para juntar NFe com seu protocolo na hora de gerar o XML de distribui��o.
          ...
          * Agora vamos pegar o protocolo de todas as notas enviadas no lote, nota a nota.
            For I = 1 TO oRetAutorizacao.Result.GetProtNfeCount()
              ...
              * Salvar XML de distribuicao das notas enviadas na pasta informada  
			    Try
                    ...
                    * Gravar o XML de distribui��o em uma pasta
                    ...
                    * Demonstrar a chave da NFe pegando do protocolo retornado
   		            ...
   		            * Pegar a string do XML de distribui��o para gravar na base de dados
		              docProcNFe = oAutorizacao.GetNFeProcResults(oProtNFe.InfProt.chNFe)
		              
		              MESSAGEBOX(docProcNFe)
		              
		            * Gravar o XML de distribui��o com um nome diferente do sugerido pela DLL
		            *  
			        * Nome do XML de distribui��o gerado pela DLL segue o o seguinte padr�o:
			...
			Next I	        	        
	     ELSE
	        * Pode ser que o lote j� tenha sido rejeitado na sua totalidade, tem que fazer os devidos tratamentos
	        * Ou pode ser que o lote esteja em processamento, ent�o tem que efetuar a consulta recibo novamente mais tarde
	     ENDIF
	  endif
