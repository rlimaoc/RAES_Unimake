...
CHNota = Request.QueryString("CHNota")
If trim(CHNota) = "" then
   CHNota  = Request.form("CHNota")
End If
...
With DetEventoManif
    ...
    .XJust = "Justificativa para manifestacao da NFe de teste"
End With
              
With InfEvento
    .DetEvento = DetEventoManif
    .COrgao = UFBrasil.AN
    .ChNFe = CHNota
...
''Gravar o XML de distribuicao se o evento foi homologado
If (RecepcaoEvento.result.CStat = 128) Then ''128 = Lote de evento processado com sucesso
  '  CStat = RecepcaoEvento.result.GetEvento(0).InfEvento.CStat
	CStat = RecepcaoEvento.result.RetEvento(0).InfEvento.CStat
    ...
    '' 155: Evento de Manifestacao homologado fora do prazo permitido
                        
    If CStat = 135 Or CStat = 136 Or CStat = 155 Then
            RecepcaoEvento.GravarXmlDistribuicao (Trim(Pathe) & "Retorno\")
         Else ''Evento rejeitado
            RecepcaoEvento.GravarXmlDistribuicao (Trim(Pathe) & "Erro\")
    End If
           Metext.text = RecepcaoEvento.Result.RetEvento(0).InfEvento.CStat & " - " & RecepcaoEvento.Result.RetEvento(0).InfEvento.XMotivo
            Exit Sub
Else
       Metext.text = "Lote nao processado. Stat = " & RecepcaoEvento.result.CStat
       Exit Sub            
